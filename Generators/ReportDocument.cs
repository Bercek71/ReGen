...
public class ReportDocument : IDocument
{
    ...
    {
        if (!File.Exists(chartImagePath)) throw new ArgumentException("Chart image file is invalid");
                ...
                col.Item().Row(row =>
                {
                    row.ConstantItem(150).Border(1, Colors.Black).Row(row1 =>
                    {
                        row1.ConstantItem(150).Padding(5).Column(infoCol =>
                        {
                            ...
                            {
                                testNumeberRow.RelativeItem()
                                    .Column(labelCol => labelCol.Item().Text("Test:").ExtraBold());
                    ...
                    row.ConstantItem(190).AlignRight().Border(1, Colors.Black).BorderLeft(0).Padding(5).Row(infoRow =>
                    {
                        infoRow.RelativeItem().Column(infoCol =>
                        {
                            ...
                            {
                                maintenanceCountRow.RelativeItem()
                                    .Column(labelCol => labelCol.Item().Text("Maintenance count:"));
                                maintenanceCountRow.RelativeItem().Column(labelCol =>
                                    labelCol.Item().Text(Settings_Designer.Default.MaintenanceCount.ToString()));
                            });
                        });
                    });
                });
